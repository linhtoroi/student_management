<?php
    ...
    if ($conn->query($sql1) === TRUE && $conn->query($sql) === TRUE)  {         // Thực hiện thêm record
        header("Location: /student_management/listOfUsers.php?account=".$account, true, 301); // Hoàn thành thì điều hướng lại về trang riêng của account đó
    } else {
        header("Location: /student_management/editForTeacher.php", true, 301);  // Không hoàn thành thì điều hướng lại về trang edit                               
    }   
